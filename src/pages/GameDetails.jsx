  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white">
      ...
      <div className="relative">
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 left-4 text-white z-10 transition-transform duration-200 hover:scale-110 active:scale-95 cursor-pointer"
        >
        ...
        <button
          ...
          className="absolute top-4 right-4 z-10 transition-transform duration-200 hover:scale-110 active:scale-95 cursor-pointer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
            viewBox="0 0 24 24" fill={favorited ? "red" : "none"}
                ...
                {(() => {
                  ...
                  return game.platforms?.map((p) => {
                    ...
                    if (logoKey && !shown.has(logoKey)) {
                      shown.add(logoKey);
                      return (
                        <img key={logoKey} src={storeLogos[logoKey]} alt={logoKey}
                          title={logoKey.replace("_", " ")} className="w-5 h-5 object-contain" />
                      );
                    }
                    return null;
                  }).filter(Boolean);
